...
const StatusBadge = ({ isProcessed }: StatusBadgeProps) => {
  if (isProcessed) {
    return (
      <div className="flex items-center gap-2 text-emerald-400 text-xs font-medium bg-emerald-400/10 px-2 py-1 rounded-full w-fit border border-emerald-400/20 ">
        <div className="w-1.5 h-3 rounded-full bg-emerald-400"></div>
        Ready
      </div>
    );
  }
